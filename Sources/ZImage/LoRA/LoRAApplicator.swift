...
public struct LoRAApplicator {
  ...
  public static func mergeWeights(
    ...
    logger?.info(
      "Merging LoRA weights with scale=\(scale), alpha=\(loraWeights.alpha), rank=\(loraWeights.rank), effective_scale=\(effectiveScale)"
    )
    ...
    for (key, module) in transformer.namedModules() {
      ...
      if let quantizedLinear = module as? QuantizedLinear {
        ...
        guard let alignedDelta = alignShape(delta, to: dequantizedWeight.shape) else {
          logger?.debug(
            "LoRA delta shape \(delta.shape) doesn't match dequantized \(dequantizedWeight.shape) for \(key)")
          continue
        }
  ...
  public static func removeFromWeights(
    ...
    for (keyPath, (down, up)) in loraWeights.weights {
      ...
      guard let currentWeight = restored[weightKey],
        ...
        let alignedDelta = alignShape(delta, to: currentWeight.shape)
      else {
        continue
      }
  ...
  private static func applyLoKrInternal<T: Module>(
    ...
    for (key, module) in transformer.namedModules() {
      ...
      if expectedOut != dims.out || expectedIn != dims.in {
        logger?.debug(
          "Skipping LoKr for \(key): kron (\(expectedOut)x\(expectedIn)) vs weight (\(dims.out)x\(dims.in))")
        continue
      }
  ...
  public static func applyDynamically<T: Module>(
    ...
    for (key, module) in transformer.namedModules() {
      ...
      guard
        ...
        logger?.debug(
          "Skipping LoRA for \(key): down=\(pair.down.shape) up=\(pair.up.shape) vs weight (\(dims.out)x\(dims.in))")
        continue
      }
